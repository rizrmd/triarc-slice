...
func _end_drag():
	...
	if valid_drop:
		...
		# Check if this action needs enemy targeting
		if target_rule == "enemy":
			# Emit signal to enter targeting mode - player must click an enemy
			var caster = dropped_on as Hero
			card_awaiting_target.emit(self, caster)
			# Don't emit card_drag_ended yet - wait for target selection
			return
